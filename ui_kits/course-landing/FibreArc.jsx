// FibreArc — WebGL background (Originkit), adapted from TSX to plain JSX for this
// page's in-browser Babel pipeline. Curved light strands radiating from a corner,
...
function arFibreCompile(gl, type, src) {
...
function arFibreParseColor(input, fb) {
  if (!input) return fb;
...
function arFibreNum(v, fb) { return typeof v === 'number' && isFinite(v) ? v : fb; }
function arFibreClamp(v, lo, hi) { return v < lo ? lo : v > hi ? hi : v; }

const arFibreBundleDefaults = { curve: 150, spread: 100, thickness: 100, comb: 170 };

function FibreArcBase(props) {
  const {
    style,
    background = '#01030A',
    baseColor = '#1B4FD8',
  ...
  const bundle_ = { ...arFibreBundleDefaults, ...(bundle || {}) };

  const canvasRef = useRef(null);
  const sizeRef = useRef({ w: 0, h: 0 });
  sizeRef.current = { w: arFibreNum(width, 0), h: arFibreNum(height, 0) };
  ...
  vRef.current = {
    ...
    accentColor,
    highlight,
    density: arFibreClamp(arFibreNum(density, 26), 4, 26),
    speed: arFibreClamp(arFibreNum(speed, 50), 0, 100) / 50,
    direction: (arFibreClamp(arFibreNum(direction, 0), 0, 360) * Math.PI) / 180,
    hover: arFibreClamp(arFibreNum(hover, 100), 0, 200) / 100,
    reach: arFibreClamp(arFibreNum(reach, 30), 5, 100) / 100,
    curve: arFibreClamp(arFibreNum(bundle_.curve, 150), 20, 400) / 100,
    spread: arFibreClamp(arFibreNum(bundle_.spread, 100), 0, 300) / 100,
    thickness: arFibreClamp(arFibreNum(bundle_.thickness, 100), 20, 400) / 100,
    comb: arFibreClamp(arFibreNum(bundle_.comb, 170), 0, 400) / 100,
  };
  ...
  useEffect(() => {
    ...
    const vs = arFibreCompile(gl, gl.VERTEX_SHADER, arFibreVert);
    const fs = arFibreCompile(gl, gl.FRAGMENT_SHADER, arFibreFrag);
    if (!vs || !fs) return;
    ...
    const render = (now) => {
      ...
      const dpr = Math.min(window.devicePixelRatio || 1, arFibreMaxDpr);
      ...
      const c_uBg = arFibreParseColor(v.background, [0.004, 0.012, 0.039]);
      gl.uniform3f(u('uBg'), c_uBg[0], c_uBg[1], c_uBg[2]);
      const c_uBase = arFibreParseColor(v.baseColor, [0.106, 0.31, 0.847]);
      gl.uniform3f(u('uBase'), c_uBase[0], c_uBase[1], c_uBase[2]);
      const c_uAccent = arFibreParseColor(v.accentColor, [0.435, 0.784, 1.0]);
      gl.uniform3f(u('uAccent'), c_uAccent[0], c_uAccent[1], c_uAccent[2]);
      const c_uHigh = arFibreParseColor(v.highlight, [1.0, 1.0, 1.0]);
      gl.uniform3f(u('uHigh'), c_uHigh[0], c_uHigh[1], c_uHigh[2]);
      gl.uniform1f(u('uStrands'), v.density);
      gl.uniform1f(u('uReach'), v.reach);
      gl.uniform1f(u('uDir'), v.direction);
      gl.uniform1f(u('uCurve'), v.curve);
      gl.uniform1f(u('uSpread'), v.spread);
      gl.uniform1f(u('uThin'), v.thickness);
      gl.uniform1f(u('uComb'), v.comb);
    ...
    const track = (e) => {
      ...
      ptrRef.current.tx = arFibreClamp((e.clientX - r.left) / r.width, 0, 1);
      ptrRef.current.ty = arFibreClamp((e.clientY - r.top) / r.height, 0, 1);
...
function FibreArc(props) {
  return React.createElement(FibreArcBase, { bundle: { comb: 170, curve: 20, spread: 100, thickness: 100 }, ...props });
}

window.FibreArc = FibreArc;
